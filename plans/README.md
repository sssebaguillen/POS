...
| 014  | Imágenes: decidir optimización Next/Vercel vs `unoptimized` global documentado (11 componentes) | P3 | S | — | DONE (2026-06-13, **variante B** — decisión del dueño: sin plan Pro de Vercel, activar optimización impondría límite de cuota). `unoptimized: true` global en next.config.ts con comentario de decisión; prop `unoptimized` removido de los 11 componentes (7 catálogo + pos/ProductPanel + 2 inventory + orders/OrderDetailPanel). Comportamiento idéntico a antes (todo seguía sin optimizar) pero fuente única de verdad. typecheck/lint/build verdes; smoke visual de bajo riesgo (sin cambio de comportamiento). Revertir = borrar 1 línea si se sube a Pro |
