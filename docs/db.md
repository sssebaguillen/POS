...
| `get_promo_impact(p_business_id, p_from?, p_to?)` | Impacto de promos para el widget de `/stats` — `{totals, data[]}` por promoción (ventas, unidades, facturado neto, descuento resignado) leyendo `sale_items` en vivo (no snapshots: cubren hasta ayer y no desglosan por promo). Líneas NETAS: `revenue = SUM(si.total)`, `discount_total = SUM(si.promo_discount)`. `totals` incluye `total_sales_count` para el share de ventas con promo. `assert_tenant` + `GRANT EXECUTE TO authenticated`. Mig. `20260612_02`. |
