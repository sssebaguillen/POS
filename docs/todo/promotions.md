...
| **F3 — UI `/promotions`** | Página con chips Activas/Programadas/Vencidas, New/Edit modal (selector de tipo + preview + aviso de solapamiento), sidebar, entity type en `/activity`. | ✅ 2026-06-10. `/promotions` (gate `inventory_read` en proxy, escrituras `inventory_write`), `PromotionsView` (chips de estado + tabla + pausar/reanudar/archivar), `PromotionModal` compartido new/edit (4 tipos en lenguaje natural → mapeo N/K/P, picker de alcance, vigencia, preview, aviso ámbar de solapamiento por intersección de productos cubiertos). `/activity`: entity `promotion` + `PromotionSummary`/`PromotionDiff` + tone `_archived`. |
| **F4 — Catálogo** | RPCs devuelven `original_price` + label de promo (+ `ends_at` para countdown "termina en 2 días"), badge + tachado en `ProductCard`, sección "Ofertas" + chip, `create_catalog_order` aplica promo, passthrough en conversión. | ✅ 2026-06-10 (mig `20260610_01_promotions_catalog`). `get_catalog_products` devuelve sale_price con promo + original_price + campos crudos (label se arma client-side con `promoBadgeLabel` — una sola implementación); detalle/selector con promo por variante; checkout re-precia con promo (verificado en vivo: 20% y 2x1 al centavo); conversión arrastra promotion_id/promo_discount. UI: badges + tachado + countdown en cards/lista/detalle, sección "Ofertas" destacada (se oculta con filtros activos; nota: se omitió el chip de filtro "Ofertas" — la sección cubre el caso), carrito del catálogo espeja el descuento de cantidad para que el total mostrado == cobrado. |
| **F5 — Cierre** | Invariante nuevo en reconciliación (`total línea = qty×unit − promo_discount` cuando hay promo... ver nota*), smoke E2E, docs (CLAUDE.md, db.md, conventions.md, backlog). | ✅ 2026-06-10. R1 actualizado (regla dual auto-contenida, sin depender del `kind` editable) + R12a/b/c nuevos en `06-reconciliacion.sql` — suite en verde (0 violaciones). Smoke server-side al centavo: checkout catálogo (20% y 2x1) + venta POS (3x2 vía claims simulados), todo en ROLLBACK. `npm run build` ✅. Docs: CLAUDE.md (sección + regla 36 + key files), db.md, conventions.md, backlog. **Pendiente: smoke UI en browser** (crear promo de cada tipo desde `/promotions` y verla en POS/catálogo). |
