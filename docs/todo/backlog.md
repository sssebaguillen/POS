...
8. **Orden de prioridad:** (1) quick wins (docs/lint/env); (2) deuda técnica **no-money-path** con scope claro; (3) features `🤖 SCHEDULE-OK` no-money-path. Los items money-path (CartPanel/POSView/cash/inventario que muten stock, auth, onboarding) quedan bloqueados por la regla 5. **Nota (2026-06-19):** al día de hoy la cola elegible `🤖 SCHEDULE-OK` quedó **prácticamente agotada** (migración Phosphor, segmentación POS/catálogo #22, tarjeta promos, lista catálogo, los fetch `.then()` de inventario, supplier dropdown #33 — todos cerrados). Lo que resta es money-path (E2E, sin DB en el runner) o `🔒 NEEDS-OWNER`. Si no encontrás item elegible: aplicá la regla 10 (cerrá la corrida; propuestas como plus acotado).
...
10. **Cuando se agote el trabajo elegible: cerrá la corrida; las propuestas son un PLUS acotado, no un relleno.** Lo importante es **cerrar el trabajo elegible** (reglas 8–9). Si ya no quedan items `🤖 SCHEDULE-OK` sin bloquear, **no hace falta gastar el resto de la sesión** — está bien terminar temprano. Si querés, dedicá **un rato acotado** (no todos los tokens de la corrida) a estudiar la app y dejar **1–3 propuestas fundamentadas** para el dueño en la sección "💡 Propuestas del agente automático" de abajo, en un PR aparte (`docs(backlog): propuestas del agente [fecha]`). Prioridad: **(a) fixes / bugs / calidad / seguridad** y **(b) features nuevas valiosas** primero; (c) limpieza / deuda; (d) opcional naming/branding. Cada propuesta: qué observaste (archivo:línea), por qué importa, propuesta concreta. **NO las implementes.** **No dupliques** propuestas ya anotadas (revisá la sección antes de agregar) — si no tenés nada nuevo y de valor que decir, **no agregues por agregar**: cerrá la corrida.
...
- **[2026-06-20] [calidad/observabilidad] Las páginas de detalle de `/stats` (Server Components) descartan el `{ error }` de sus RPCs y degradan a cero/vacío en silencio.** Todos los Server Components de stats destructuran **solo `data`** del `supabase.rpc(...)` y caen a `?? []` / totales en cero cuando la RPC falla, **sin chequear `error`** (regla 5 de CLAUDE.md, "chequear `error` en toda RPC"): `stats/trends/page.tsx:29` (`get_period_comparison`), `stats/breakdown/page.tsx:30,39` (`get_sales_by_brand_detail`/`get_sales_by_category_detail`), `stats/operators/page.tsx:27` (`get_sales_by_operator_detail`), `stats/payment-methods/page.tsx:26` (`get_sales_by_payment_detail`), `stats/heatmap/page.tsx:29` (`get_sales_heatmap`), `stats/top-products/page.tsx:32-40`, `stats/inventory-health/page.tsx:62-70`, `stats/report/page.tsx:61-68`, y el prefetch server de `stats/page.tsx:31-38`. **Por qué importa:** en el free-tier un cold start o `statement_timeout` hace que la RPC devuelva `{ data: null, error }`; con el `?? []` el operador ve "$0 / sin ventas" para el período **sin ninguna señal de que la consulta falló**, y podría leerlo como dato real ("no vendí nada") — una métrica de negocio falsa, que es peor que un error explícito. Contrasta con el `/stats` agregado (`StatsView`, client + React Query) que SÍ maneja error/loading en el refetch; las drill-downs no. Además no hay `(app)/error.tsx` ni `stats/error.tsx`: un throw hoy escalaría hasta `global-error.tsx` (pantalla completa). **No money-path** (RPCs de solo lectura → `tsc`+`lint`+`build`, sin E2E). **Propuesta:** (a) chequear `error` en cada RPC y ante fallo `throw` (consistente con regla 5) + sumar un `error.tsx` granular en `(app)/stats/` (o `(app)/`) con reintento, en vez de degradar a cero; o (b) mínimo, pasar una bandera `loadError` a cada *View* para mostrar un banner "no se pudieron cargar las métricas" en lugar de ceros. Un componente/página por PR.
