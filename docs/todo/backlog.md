...
- **[2026-06-18] `set-state-in-effect` en `OnboardingChecklist.tsx` — ¿lo encara el agente o queda para vivo?** El backlog lo listaba como "seguro (el agente puede)", pero el componente **muta `profiles.onboarding_state`**: el efecto de auto-completar onboarding (`OnboardingChecklist.tsx:111-142`, `update profiles.onboarding_state = {completed:true}`) y los launchers (`:144-177`). Eso es **camino de onboarding** → regla 5 exige E2E verde antes de cerrar, y el runner no tiene DB para correrlo. Además el warning a tocar (`:80-82`, `void load()` en efecto) es un fetch-on-mount cuya conversión "limpia" es migrar a React Query (`useQuery`), refactor que altera el orden de efectos del onboarding. **Opciones:** (a) dejarlo para una sesión en vivo con E2E (recomendado); (b) autorizar explícitamente al agente a tocar solo el `void load()` → `useQuery` con la garantía de que el dueño valida el flujo de onboarding a mano; (c) dejar un `eslint-disable` puntual documentado y cerrarlo del backlog. **El agente lo deja sin avanzar.**
- **[2026-06-18] `OrdersView.tsx` — hidratación de `viewedIds` (último `set-state-in-effect`).** El efecto `:91-95` lee `localStorage` a estado tras el mount (patrón `mounted`, regla 25) para no pintar el resaltado ámbar de "pedido no visto" durante SSR. Migrarlo a `useSyncExternalStore` (como #21) **cambiaría el comportamiento de flash**: con `getServerSnapshot` vacío, todos los pendientes saldrían ámbar en el primer paint y se corregirían al hidratar (flash). **Opciones:** (a) dejarlo (es el patrón SSR-safe correcto, mismo grupo que theme/sidebar/CatalogThemeProvider → NEEDS-OWNER); (b) aceptar el flash y migrar a `useSyncExternalStore` con caché de snapshot; (c) `eslint-disable` puntual. **El agente lo deja sin avanzar** (mismo criterio que los otros `mounted`).
...
- **🤖 SCHEDULE-OK (de a uno) — Refactor selectivo de `react-hooks/set-state-in-effect`** (hoy en `warn`, decisión 2026-06-12, comentado en `eslint.config.mjs`): reset-de-estado-al-abrir-modal + suscripciones externas. Con la suite de tests como red se encara de a poco. Son además los componentes que React Compiler no optimiza. **Un componente por PR** (no batch — facilita revisión y aísla regresiones); los que tocan el camino del dinero exigen E2E verde antes de cerrar.
  ...
  - **Pendientes — patrón `mounted` (regla 25, NO refactorizar a la ligera):** `shared/theme.tsx` (1), `sidebar.tsx` (1), `catalog/CatalogThemeProvider.tsx` (1). El `setMounted(true)` en efecto ES el patrón SSR-safe que la regla 25 manda. Convertirlos cambia la estrategia anti-mismatch → **decisión del dueño** antes de tocar (¿migrar a `useSyncExternalStore` también acá, o dejarlos con un `eslint-disable` puntual y documentado?).
