...
- **🤖 SCHEDULE-OK — Mismo patrón de "error tragado" en `customers` y `providers` (Server Components de solo lectura).** `(app)/customers/page.tsx:14` (`from('customers')`) y `(app)/expenses/providers/page.tsx:16` (`from('suppliers')`) destructuran solo `data` y caen a `?? []` cuando la query falla → lista vacía en silencio (idéntico al bug de stats ya resuelto). **No money-path** (lecturas puras). **Fix (mismo patrón que stats):** chequear `error` y lanzar en cada página + agregar un `error.tsx` granular (en `(app)/` o por sub-ruta) espejo de `stats/error.tsx`. Verif: `tsc` + `lint` + `build`. Un PR.
...
- **🤖 SCHEDULE-OK — Ficha de cliente: estado de cuenta + saldo (roadmap 2.1, reads-only, SIN migración).** Hoy `CustomerView` solo muestra el saldo en la fila; falta una vista de detalle con el **estado de cuenta** (la cuenta corriente queda a medio servir: no se le puede mostrar al cliente "esto debés + estos movimientos"). **La data ya existe** — no hay que crear RPC ni migración:
