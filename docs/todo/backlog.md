...
- **~~🤖 SCHEDULE-OK — Higiene de tokens en el catálogo público (cero colores hardcodeados)~~ ✅ (re-verificado 2026-06-20 — sin cambios necesarios).** Barrido fresco de `components/catalog/**` (palette Tailwind completa + `white`/`black` + hex/rgb + `style={{…color}}`). Los únicos colores "crudos" que quedan son legítimos y no se tocan:
  ...
  2. **`bg-black/65` + `text-white` en `ProductCard.tsx:67,268`** (badge de countdown de promo y botón "Ver variantes", ambos **sobre la foto del producto**). Son **scrims sobre imagen, NO temáticos**: deben verse igual en claro/oscuro, y son **convención app-wide** (`bg-black/NN` aparece en 13 archivos: scrims de modal vía `dialog.tsx`/`sheet.tsx`, overlays de imagen). No existe token `--overlay` y la regla prohíbe inventar uno; tokenizarlos a `bg-foreground`/`bg-background` los **invertiría** en dark mode y rompería el scrim sobre foto.

  Sin commits a `components/catalog/` desde 2026-06-16 → la verificación previa (2026-06-17, líneas 276-277) sigue vigente. **Nada que cambiar** — cualquier edición sería un no-op visual o rompería el scrim. (Solo doc, sin tocar código.)
