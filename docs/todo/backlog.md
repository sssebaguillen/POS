...
- **[2026-06-19] Migración Lucide→Phosphor — Phosphor rompe el build en Server Components (`createContext is not a function`)** — al ejecutar el swap del área 1 del spec (`sidebar` + `shared/*` + `ui/*` + `app/**`), `tsc` y `lint` pasan pero `npm run build` **falla al recolectar page data** de las páginas que son **Server Components** e importan iconos directo: `app/auth/confirm/page.tsx`, `app/(auth)/email-confirmed/page.tsx`, `app/catalogo/not-found.tsx` (`TypeError: (0 , b.createContext) is not a function`). Causa: el entry principal `@phosphor-icons/react` evalúa `IconContext = createContext(...)` al cargar el módulo, y `createContext` no existe en el entorno RSC → cualquier Server Component que importe de ahí rompe el build. **`lucide-react` no tenía este problema** (sus iconos no usan context al cargar), por eso el swap in-place 1:1 que pide el spec no es suficiente. El spec (`FULL_MIGRATION_SPEC.md`) prescribe importar **siempre** `from '@phosphor-icons/react'` y no contempla este caso. **La regla "build verde en cada PR" del propio spec es inalcanzable sin resolver esto**, y la decisión afecta a **toda** la migración (cada área tiene Server Components que usan iconos: páginas de `/stats`, `/catalogo`, layouts, etc.) → no lo asumo, lo flagueo. **Opciones:** (a) **Server Components importan de `@phosphor-icons/react/dist/ssr`** (variante SSR oficial, sin context — existe en el paquete v2.1.10) y los Client Components siguen con `@phosphor-icons/react`; mecánico pero hay que distinguir por archivo (`'use client'` o no). (b) **Importar TODO de `@phosphor-icons/react/dist/ssr`** de forma uniforme (funciona en server y client; solo se pierde el `IconContext` global, que esta migración no usa) — un solo origen, sin clasificar archivos; es la opción más simple y robusta. (c) Convertir las pocas páginas Server Component que usan iconos a `'use client'` (cambia el runtime de esas páginas, no recomendado). **Recomiendo (b).** Decidido esto, retomo la migración área por área. Mientras tanto **NO avanzo el swap** (el área 1 quedó revertida; nada pusheado).
...
> ⚠️ **BLOQUEADA pendiente de decisión (ver ⚠️ Preguntas, [2026-06-19]):** el swap del área 1 destapó que `@phosphor-icons/react` (entry principal) **rompe el build en Server Components** (`createContext is not a function`), cosa que `lucide-react` no hacía. Afecta a toda la migración. Hay que decidir el origen de import para Server Components (recomendado: importar todo de `@phosphor-icons/react/dist/ssr`) antes de retomar. El agente revirtió el área 1 y no pusheó nada del swap.
