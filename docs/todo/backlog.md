...
- **Bulk handlers de inventario no chequean `data.success` de la RPC** (hallazgo 2026-06-01) — `handleBulkActivate/Deactivate/SetCatalog/ChangeCategory/ChangeBrand` en `InventoryPanel.tsx` solo verifican el error de transporte (`if (error)`) y luego leen `data.updated`. Si la RPC devuelve `{success:false, error}` (p. ej. permisos insuficientes, contexto inválido) NO viene por `error` sino dentro de `data` → el handler sigue de largo y muestra "0 productos actualizados" en vez del error real (falla silenciosa). Las RPC `SECURITY DEFINER` ya devuelven ese shape; falta surfacearlo. Endurecer: chequear `result?.success === false` (o que `data` traiga `error`) y mostrarlo vía `translateDbError`. No urgente (el happy-path funciona), pero confunde si un sub-operador sin `stock_write` intenta una acción bulk.
