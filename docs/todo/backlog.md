...
8. **Orden de prioridad:** (1) quick wins (docs/lint/env); (2) deuda técnica **no-money-path** con scope claro; (3) features `🤖 SCHEDULE-OK` no-money-path. Los items money-path (CartPanel/POSView/cash/inventario que muten stock, auth, onboarding) quedan bloqueados por la regla 5. **Nota (2026-06-20):** la cola se re-sembró con 2 items elegibles frescos (ver "Deuda derivada de la sesión"): **(1)** error tragado en `customers`/`providers` (mismo patrón que el fix de stats ya cerrado), y **(2)** higiene de tokens en el catálogo. Ambos no-money-path, scope claro. Tomalos primero (reglas 8–9). Cuando se vuelvan a agotar, NO cierres con una propuesta de relleno: invertí en el **modo propuestas ampliado** de la regla 10 (features faltantes, estadísticas, diseño/motion con las skills). El resto del backlog sigue siendo money-path (E2E, sin DB en el runner) o `🔒 NEEDS-OWNER`.
