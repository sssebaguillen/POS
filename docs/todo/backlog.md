...
- **[2026-06-19] Migración Phosphor — área 6 (money-path: `pos/*` + `cash-sessions/*`)** — el spec (`FULL_MIGRATION_SPEC.md` §0/§4) la define como "PR aparte, ⚠️ MONEY-PATH: el agente lo prepara, el dueño valida E2E + QA visual y mergea". Pero el contrato del agente automático (regla 5) bloquea las MUTACIONES del camino del dinero para el agente porque el runner **no tiene DB → no puede correr E2E**. El swap es inerte (solo iconos, cero lógica), pero igual toca archivos del POS/caja. **La duda:** ¿querés que prepare el PR de área 6 (swap mecánico, lo verifico con tsc+lint+build, lo dejás para tu E2E+QA y lo mergeás vos), o lo dejo enteramente para una sesión en vivo? **Opciones:** (a) lo preparo marcado `⚠️ MONEY-PATH` (como invita el spec) — vos corrés E2E y mergeás; (b) lo dejo fuera del schedule, lo hacés en vivo. Áreas 1–5 ya están en PRs (#34/#35 tuyos + #37/#38/#39 del agente); área 6 es lo único que falta para cerrar la migración y poder quitar `lucide-react` del `package.json` (checklist §5).
  - **Nota operativa (no bloqueante):** las áreas 3/4/5 del agente (#37/#38/#39) y el fix de lint (#36) **dependen de #34** (que instala `@phosphor-icons/react` en `package.json`) — igual que #35, esas ramas no tocan `package.json`. **Mergear #34 primero**; hasta entonces el CI de typecheck de #35/#37/#38/#39 queda rojo por no resolver el import (esperado). #36 deja el job de lint verde en paralelo.
...
- **🤖 SCHEDULE-OK — Manejo de error en fetches `.then()` sin `.catch()` (propuesta #1, acotada).** Dos `useEffect` cargan datos vía `supabase.rpc(...).then(...)` sin manejo de error: `inventory/VariantEditor.tsx:213` (`get_attribute_types`, traga el error en silencio) e `inventory/EditProductModal.tsx:139` (`get_product_with_variants`, si el RPC rechaza el `setLoading(false)` nunca corre → spinner infinito). **Fix:** migrar SOLO el fetch de lectura a React Query (`useQuery`, patrón de #17/#18) o, mínimo, `.catch()` que resetee loading + toast. **NO tocar la lógica de guardado/mutación.** **Un componente por PR.** **`pos/ProductPanel.tsx:259` queda EXCLUIDO** (mismo bug pero es POS/money-path → bloqueado por E2E, regla 5 → flaguear). Verif: `tsc`+`lint`+`build`.
- **🤖 SCHEDULE-OK — `SupplierSelectDropdown.tsx`: usar `create_supplier` RPC + manejar error del fetch (propuestas #32, un solo PR).** Mismo archivo, dos arreglos: **(1)** `handleCreate` (~:58-71) crea proveedores con `.insert` directo → reemplazar por `supabase.rpc('create_supplier', {...})` (chequear `data.success`/`error`, patrón de `SuppliersPanel.tsx:74`), así pasa por el guard de permiso `expenses` + `log_audit_event` (cierra el gap de auditoría, reglas 31/32). **(2)** el fetch de proveedores (~:32-40) usa `.then()` sin `.catch()` → migrar a `useQuery` (maneja error/loading, patrón de #29/#30). **No money-path.** Verif: `tsc`+`lint`+`build`.
- **🤖 SCHEDULE-OK — Selector de lista de catálogo: mostrar nombre + multiplicador base.** En `/settings` (`SettingsForm.tsx`), el selector de "lista de precios del catálogo" hoy muestra solo el `name` de cada `price_list`. **Fix:** mostrar también el multiplicador base junto al nombre (ej. `Mayorista (×1.30)`), tomando `multiplier` de la lista ya disponible en el fetch existente (sin fetch nuevo). Solo UI. Verif: `tsc`+`lint`+`build`.
- **🤖 SCHEDULE-OK — `acknowledgeOrdersSeen` no chequea el error del RPC (propuesta #2).** `orders/UnreadBadge.tsx:14-17`: `await supabase.rpc('mark_catalog_orders_read')` sin leer `{ error }`. **Fix:** `const { error } = await supabase.rpc(...); if (error) console.error(...)`, consistente con `useUnreadOrdersCount` (línea 27-28). Trivial. Verif: `tsc`+`lint`+`build`.
