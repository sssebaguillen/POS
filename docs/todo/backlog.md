...
8. **Orden de prioridad:** (1) quick wins (docs/lint/env); (2) deuda técnica **no-money-path** con scope claro; (3) features `🤖 SCHEDULE-OK` no-money-path. Los items money-path (CartPanel/POSView/cash/inventario que muten stock, auth, onboarding) quedan bloqueados por la regla 5. **Nota (2026-06-20):** la cola tiene 3 items elegibles frescos (ver "Deuda derivada de la sesión"): **(1)** error tragado en `customers`/`providers`, **(2)** higiene de tokens en el catálogo, **(3)** ficha de cliente / estado de cuenta (roadmap 2.1, reads-only, sin migración — la tabla `customer_account_movements` ya existe y se lee directo). Los tres no-money-path, scope claro. Tomalos primero (reglas 8–9). **NO son del agente** (money-path y/o decisión del dueño): devoluciones (2.2), módulo de compras (2.3), datos demo (1.1) — esos van en vivo. Cuando se vuelvan a agotar, NO cierres con una propuesta de relleno: invertí en el **modo propuestas ampliado** de la regla 10 (features faltantes, estadísticas, diseño/motion con las skills). El resto del backlog sigue siendo money-path (E2E, sin DB en el runner) o `🔒 NEEDS-OWNER`.
...
- **🤖 SCHEDULE-OK — Higiene de tokens en el catálogo público (cero colores hardcodeados).** Pasada por los componentes de `components/catalog/**` reemplazando cualquier color hardcodeado (hex/rgb/clases Tailwind de color crudas) por variables CSS / tokens existentes (`accent-colors.ts` + `globals.css`) — **sin inventar tokens nuevos** (regla: reusar/unificar, no crear uno por pantalla). Es deuda acotada y **prerequisito natural de cualquier theming futuro** del catálogo (ver "NEEDS-OWNER — Temas del catálogo", donde se la marca como lo único que paga sin riesgo desde ya). **No money-path, sin cambio de comportamiento visual** (mismo color, vía token) → `tsc` + `lint` + `build` + revisión visual. Un PR (o de a grupos de componentes si es grande).
