...
- **[2026-06-18 ✅ RESUELTA] `set-state-in-effect` en `OnboardingChecklist.tsx`** → decisión: **dejar como está, baja prioridad / a futuro.** Es onboarding (muta `profiles.onboarding_state`) → necesita E2E → NO es trabajo del agente. NO se cierra como deuda: queda trackeada en el item de `set-state` (deuda derivada de la sesión) como tarea futura. Objetivo final: poder pasar la regla `react-hooks/set-state-in-effect` de `warn` a `error`.
- **[2026-06-18 ✅ RESUELTA] `OrdersView.tsx` — `viewedIds`** → decisión: **dejar como está, baja prioridad / a futuro.** Es el patrón `mounted` (regla 25, correcto); "corregirlo" es solo un `eslint-disable` documentado junto al grupo theme/sidebar/CatalogThemeProvider. NO es trabajo del agente. Trackeada en el item de `set-state`. Mismo objetivo final: warn→error.
...
- **🤖 SCHEDULE-OK (de a uno) — Refactor selectivo de `react-hooks/set-state-in-effect`** (hoy en `warn`, decisión 2026-06-12, comentado en `eslint.config.mjs`): reset-de-estado-al-abrir-modal + suscripciones externas. Con la suite de tests como red se encara de a poco. Son además los componentes que React Compiler no optimiza. **Un componente por PR** (no batch — facilita revisión y aísla regresiones); los que tocan el camino del dinero exigen E2E verde antes de cerrar.
  - **Avance:** de los 19 originales quedan **10 warnings en 8 archivos** (al 2026-06-18). Cerrados vía `useSyncExternalStore`/render-time state: IconPickerPanel (#13), QuickEditCategoryModal (#14), dashboard (#15), PromotionModal (#16), ProductStockModal→useQuery (#17), VariantPriceModal→useQuery (#18), OffersCarousel reduced-motion (#20), useHasInsightsPermission/op_perms (#21), **OrdersView pulso de Pendientes (#23, render-time state)**.
  - **Pendientes — seguros (el agente puede):** `orders/OrdersView.tsx` (1 — el de hidratación de `viewedIds` desde localStorage; **reclasificado a patrón `mounted`**, ver abajo: convertirlo cambia el flash en SSR → decisión del dueño). `onboarding/OnboardingChecklist.tsx` (1) — **reclasificado: el componente MUTA `profiles.onboarding_state` (auto-completar onboarding + launchers), o sea camino de onboarding → exige E2E que el runner no puede correr (regla 5). Flagueado en ⚠️ Preguntas.**
  - **Pendientes — camino del dinero (flaguear, exigen E2E que el runner no puede correr):** `pos/CartPanel.tsx` (1), `pos/POSView.tsx` (1), `pos/CloseSessionModal.tsx` (1, caja), `inventory/InventoryPanel.tsx` (2).
  - **Pendientes — patrón `mounted` (regla 25, NO refactorizar a la ligera):** `shared/theme.tsx` (1), `sidebar.tsx` (1), `catalog/CatalogThemeProvider.tsx` (1), `orders/OrdersView.tsx` (1, `viewedIds`). El `setMounted(true)`/lectura de localStorage en efecto ES el patrón SSR-safe que la regla 25 manda. Convertirlos cambia la estrategia anti-mismatch → **decisión del dueño** antes de tocar (¿migrar a `useSyncExternalStore` también acá, o dejarlos con un `eslint-disable` puntual y documentado?).
  - **Pendiente — onboarding (necesita E2E, no es del agente):** `onboarding/OnboardingChecklist.tsx` (1, `void load()`) — muta `profiles.onboarding_state` → migrar a `useQuery` exige validar el flujo de onboarding a mano. Sesión en vivo.
  - **[DECISIÓN 2026-06-20 — sesión en vivo] Cola agotada para el agente headless; se deja como está.** Revisados los 10 warnings restantes uno por uno: 4 son patrón `mounted` (decisión del dueño, ver abajo), 3 money-path (POS/caja, E2E), 1 onboarding (E2E), y los 2 de `InventoryPanel` (`:111` deep-link, `:200` reset de paginación) son UI-pura (no mutan stock) **pero** el scroll infinito no está cubierto por el E2E → refactor con riesgo de regresión no atrapado → **no califican estricto para SCHEDULE-OK**. El agente automático tenía razón en saltearlos. El único camino para avanzar es una **decisión del dueño sobre el grupo `mounted`** (abajo) que destrabe pasar la regla a `error`. No se fuerza trabajo marginal.
  - **🎯 Objetivo final (baja prioridad):** cerrar estos restantes (mounted group + onboarding) — la mayoría con `eslint-disable` documentado, OnboardingChecklist con refactor en vivo — para poder pasar la regla `react-hooks/set-state-in-effect` de `warn` a `error` en `eslint.config.mjs` (cierra la clase de bug para siempre). No urgente.
