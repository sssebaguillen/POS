...
- **Endpoints anónimos del catálogo** — `/api/catalog/orders`: rate limit por IP **ya existe** (5/hora por IP+slug) y re-precia server-side en `create_catalog_order`. Falta revisar inyección vía payload anónimo.
