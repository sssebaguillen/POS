-- Mercadería Expense Variants
-- Extends the mercadería expense flow to support product variants:
--   - expense_items.variant_id column
--   - search_expense_products RPC (variant-expanded search)
--   - get_mercaderia_expense_items RPC (items with resolved variant labels)
--   - create_mercaderia_expense: variant-aware stock/cost writes
--   - update_mercaderia_expense: variant-aware delta reconciliation via composite key
--   - delete_expense: variant-aware stock revert

-- 1. Add variant_id to expense_items
ALTER TABLE public.expense_items
  ADD COLUMN IF NOT EXISTS variant_id uuid REFERENCES public.product_variants(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_expense_items_variant_id
  ON public.expense_items(variant_id) WHERE variant_id IS NOT NULL;

-- 2. search_expense_products: flat result set for mercadería product search
--    Non-variant products return one row; variant products return one row per active variant.
CREATE OR REPLACE FUNCTION public.search_expense_products(
  p_business_id uuid,
  p_term        text,
...
GRANT EXECUTE ON FUNCTION public.search_expense_products(uuid, text, int) TO authenticated;

-- 3. get_mercaderia_expense_items: loads items for an expense with resolved variant labels
CREATE OR REPLACE FUNCTION public.get_mercaderia_expense_items(
  p_expense_id  uuid,
  p_business_id uuid
)
RETURNS TABLE (
...
-- 4. create_mercaderia_expense: variant-aware
...
-- 5. update_mercaderia_expense: variant-aware, composite key (product_id, variant_id)
DROP FUNCTION IF EXISTS public.update_mercaderia_expense(uuid, uuid, text, date, uuid, text, jsonb, uuid);
...
-- 6. delete_expense: variant-aware stock revert
