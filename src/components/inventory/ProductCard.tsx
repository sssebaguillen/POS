  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 mb-3">
        <div
          className={`group/catfield flex items-center gap-1 min-w-0 rounded px-1 -mx-1 touch-manipulation ${!readOnly ? 'cursor-pointer hover:bg-primary/5' : ''}`}
        ...
        >
          ...
          {!readOnly && (
            <Pencil size={9} className="shrink-0 text-primary opacity-40 [@media(hover:hover)]:opacity-0 group-hover/catfield:opacity-60 transition-opacity" />
          )}
        </div>
        <div
          className={`group/brandfield flex items-center gap-1 min-w-0 rounded px-1 -mx-1 touch-manipulation ${!readOnly ? 'cursor-pointer hover:bg-primary/5' : ''}`}
        ...
        >
          ...
          {!readOnly && (
            <Pencil size={9} className="shrink-0 text-primary opacity-40 [@media(hover:hover)]:opacity-0 group-hover/brandfield:opacity-60 transition-opacity" />
          )}
        </div>
      </div>
      ...
      {!readOnly && (
        <div className="flex flex-col gap-1.5 mt-auto">
          <button
            ...
            className="w-full text-xs py-2 rounded-lg border border-edge text-body hover:bg-hover-bg transition-colors disabled:opacity-50 touch-manipulation"
          >
            Editar
          </button>
          <div className="grid grid-cols-2 gap-1.5">
            <button
              ...
              disabled={loadingId === product.id}
              className="text-xs py-2 rounded-lg border border-edge text-body hover:bg-hover-bg transition-colors disabled:opacity-50 touch-manipulation"
            >
            ...
            <button
              ...
              disabled={loadingId === product.id}
              className="text-xs py-2 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/50 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors disabled:opacity-50 touch-manipulation"
            >
              Eliminar
            </button>
          </div>
